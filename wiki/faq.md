...
`license_proxy` 只用于访问授权中心，不等同于用于 DMM 与 GFriends 的 `metadata.proxy`。请确认这个代理能够访问授权中心域名 `sakuramedia-license-worker.tinyping.workers.dev`。如果代理配置后仍然失败，优先确认 SakuraMedia 后端容器内是否能访问该代理。
