...
### 我收藏的片段存在哪里？删了原影片会不会一起没了？

结论：片段是独立文件，存在专门的目录里；删除来源影片**不会**删掉片段。

说明：

- 片段切出来后是一份独立的 mp4，存放在 `[media]` 的 `media_clip_root_path`（默认 `/data/media-clips`）。
- 片段与来源影片解耦：删掉来源影片资源后，片段记录、文件和番号快照都保留，仍可播放，只是封面 / 预览帧这类实时从来源缩略图解析的内容会缺失。
- 正因为它是独立资产，这个目录**必须单独挂一个持久化卷**，否则容器重建后片段会丢失。详见[视频片段收藏 → 部署须知](/guide/clips#部署须知-必须单独挂一个持久化卷)。

### 为什么圈选片段失败，或者提示片段过长？

结论：片段是同步切出来的，所以对区间长度和单次耗时都有上限。
...
- 片段合集的成员是**片段**，可以来自完全不同的影片，按你排好的顺序连续播放。
- 同一个片段可以同时放进多个合集；删片段会自动从所有合集移除，删合集不会删片段本体。
- 它和「影片播放列表」是两套独立机制：播放列表放整部影片，片段合集放片段。

### 非 JAV 视频怎么纳管？和导入影片有什么区别？

结论：用 `import-videos` 就地索引，不抓元数据、不搬文件。

说明：

- 它面向**没有番号、没有外部元数据**的普通视频，和 JAV 影片体系平行，定位是「仅播放 + 整理」。
- `import-videos` 是就地索引：只建记录、文件留在原位，标题默认取文件名，不抓 JavDB / DMM 元数据。
- 它用独立的「人物」维度（不复用女优）、复用通用标签，并支持按顺序播放的视频合集。
- 不提供订阅、自动下载、推荐、相似影片、以图搜图等 JAV 专属能力。
- 命令和参数见[常用命令 → 导入非 JAV 视频](/guide/commands#导入非-jav-视频-无番号)，功能说明见[非 JAV 视频管理](/guide/videos)。
...
- 想看视频片段怎么用： [视频片段收藏](/guide/clips)
- 想看非 JAV 视频管理： [非 JAV 视频管理](/guide/videos)
