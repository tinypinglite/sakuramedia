...
- 一个数据源激活码；外部数据源能力需要免费激活后使用，获取方式见[数据源授权](/guide/license)
...
- 至少 `4核8G`， 内存要用来运行Joytag推理模型还有图片的向量数据索引，内存占用会比较多。
- 后端所在宿主机 CPU 需要支持 `AVX` 和 `AVX2` 指令集，用于兼容`lancedb`数据库
- 运行数据尽量放在 SSD
- 媒体文件尽量放在容量盘或机械盘
...
SakuraMedia 后端服务依赖 `AVX` 和 `AVX2` 指令集。如果宿主机 CPU 不支持，后端容器可能会启动失败，或者在运行过程中出现异常退出。

在 Linux / NAS 宿主机上，可以先用下面的命令检查：
...
下面是一些常见已知不满足要求的 CPU 型号，主要用于提前排查，不是完整名单：

- Intel Celeron `J4005` / `J4025` / `J4105` / `J4115` / `J4125`
- Intel Celeron `N4000` / `N4020` / `N4100` / `N4120`
- Intel Pentium Silver `J5005` / `J5040` / `N5000` / `N5030`
- Intel Celeron / Pentium Silver Jasper Lake 常见型号，例如 `N4500` / `N5100` / `N5105` / `N6000` / `N6005`
- 较老桌面 Pentium，例如 `G4560` 等，需要按具体型号确认

可以用自己的CPU + AVX支持搜索查询，或者询问AI自己的CPU型号是否支持AVX和AVX2指令集。
...
# 不需要代理时留空即可
proxy = ""
# 授权中心 HTTP 代理；仅访问授权中心时使用，不需要代理时留空即可
license_proxy = ""
...
进入 `配置中心 → 数据源`，在“数据源授权”卡片中粘贴激活码，然后点击“激活授权”。

数据源激活只用于控制同时使用人数，不会收取费用。激活后后端会每 6 小时自动刷新一次授权；如果连续超过 24 小时无法刷新，授权会失效。授权失效只影响外部数据抓取，本地媒体库、播放和已导入内容浏览仍可正常使用。

如果授权中心无法连接，可以先查看[数据源授权](/guide/license)里的 `metadata.license_proxy` 说明。
