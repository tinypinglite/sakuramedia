...
你可以把它理解成：`sakuramedia` 负责业务，`sakuramedia-web` 负责界面，`joytag-infer` 负责图片向量化，`qdrant` 负责向量存储与检索。
...
- `joytag-infer` 是以图搜图推理服务，这里默认用 `cpu` 版本
- `qdrant` 是图片搜索向量数据库，存储缩略图向量并提供检索；存储目录 `./docker-data/image-search-index` 务必在 SSD 上
...
- `./docker-data/media-clips:/data/media-clips` 用来持久化你收藏的视频切片，**不挂这一行的话，容器重建或升级镜像后切片会全部丢失**（功能详见[切片收藏](/guide/clips)）
...
如果你的实际媒体盘路径不是 `/mnt/volume1/media`，那你只需要改这一行，把左右两边一起改掉即可。

::: danger 媒体目录必须挂载到 `/mnt` 下
无论你怎么改，**容器内路径（冒号右侧）必须以 `/mnt/` 开头**，例如 `/mnt/你的盘名/media`。

原因是：导入已有媒体时，后端的目录浏览 API 只能从 `/mnt` 这个根开始往下浏览。如果你把媒体目录挂到 `/data`、`/media` 等其他位置，在 SakuraMedia 里就根本看不到、也选不到它们，自然也无法导入历史影片。
:::


### 4. 放置 JoyTag 模型

第一次部署阶段，`joytag-infer` 先保持 `cpu` 就行，不建议再额外改成别的后端。
