...
- 很多番号在公开 BT 上根本搜不到，或者只剩零星几个做种者，自动下载的基础过滤（`seeders >= 3`）直接把它们挡在外面
- 老番、冷门番在公开源上的留存率很低，前面说的「90% 可再生」在纯 BT 环境里会明显打折
...
> 顺带一提：自动下载在挑资源时，同一候选池里本身就是 `PT` 优先于 `BT`（详见 [常见问题 → 自动下载时，系统怎么选资源？](/faq#自动下载时-系统怎么选资源)）。接了 PT 之后，系统才有「更优资源」可选。
...
- **非 JAV 视频与你自己切的片段**：[非 JAV 视频](/guide/videos)没有番号、无法从网络再生，[视频片段](/guide/clips)是你自己切出来的独立文件——这两类都不在「记番号、丢了再下」的范畴内，需要你按普通文件自己做好备份（片段目录尤其要单独挂持久化卷）。
