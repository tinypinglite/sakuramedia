...
- 媒体放在本机、NAS 挂载目录，并使用 qBittorrent 下载：安装「本地存储与 qBittorrent」。
- 媒体放在 115 网盘，并使用 115 离线下载：安装「115 网盘 Provider」。
...
精确的 v0.5.3 升级流程会自动安装本地和 115 Provider；全新部署需要按上面的步骤自行安装所需插件。插件的底层目录和配置项见[配置说明](/guide/config#plugins)，命令行管理方式见[常用命令](/guide/commands#插件管理)。
