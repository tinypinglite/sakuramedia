...
- 桌面端：已实现登录、工作台壳层、概览、发现（含顶部“女优上新”预览区块）、女优上新（发现页“更多”进入的子页面）、影片、女优、标签、时刻、播放列表、排行榜、热评、活动中心、搜索、详情、系统设置（失效媒体维护已并入为其中一个 Tab）
...
桌面端当前一级导航有十一个，分为三区呈现，「浏览」与「管理」带不可点击的分组标题：

- 概览（置顶，无分组标题）
- 浏览：发现、影片、女优、标签、时刻、播放列表、排行榜、热评
- 管理：系统设置、活动中心（失效媒体维护已并入「系统设置」页的「媒体维护」Tab）
