...
- 桌面端：已实现登录、工作台壳层、概览、发现（含顶部“女优上新”预览区块）、女优上新（发现页“更多”进入的子页面）、影片、女优、标签、时刻、播放列表、排行榜、热评、媒体维护、活动中心、搜索、详情、配置管理
...
- 已实现独立桌面/Web 页面 `/desktop/system/media-maintenance`
- 页面仅展示后端标记为失效的本地媒体，支持手动刷新、滚动触底加载更多、单条有效性复查和删除
...
- 主导航页面（`/desktop/overview`、`/desktop/library/discover`、`/desktop/library/movies`、`/desktop/library/actors`、`/desktop/library/moments`、`/desktop/library/playlists`、`/desktop/library/rankings`、`/desktop/library/hot-reviews`、`/desktop/system/media-maintenance`、`/desktop/system/activity`、`/desktop/system/configuration`）统一使用 `go`
