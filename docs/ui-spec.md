...
基础信息页中的密码修改为内联表单，提交成功后会立即退出当前登录态并返回登录页。媒体库当前以列表卡片方式展示，新增 / 编辑通过对话框完成，不提供删除入口。下载器配置依赖媒体库列表，并将 qBittorrent 使用的客户端保存路径与后端访问文件时使用的本地访问路径拆分配置。索引器通过对话框新增 / 编辑，保存时需要为每个索引器选择一个下载器；如果当前还没有下载器配置，索引器新增入口会禁用并提示先去下载器 Tab 创建下载器。播放列表 Tab 仅展示可维护的自定义播放列表，支持在当前页直接新建、编辑与删除；编辑使用对话框，删除前使用确认弹窗。
