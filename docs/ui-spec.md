...
- 移动端：已接入底部四导航（概览/影片/女优/榜单）与首页顶部 Tab（我的/关注/发现/时刻）；其中“我的”已实现搜索栏、最近添加和播放列表，其余 Tab 仍以占位页为主
- Web 端：只有基础路由入口与占位页

因此，本规范默认以桌面端为主进行描述。移动端和 Web 端暂时不具备与桌面端等价的完整视觉与交互实现；移动端当前已完成首页“我的”核心内容，但详情/播放器等能力仍是占位或提示态。
...
- 移动端子路由（当前包括 `/mobile/search`、`/mobile/search/:query`、`/mobile/search/image`、`/mobile/overview/playlists/:playlistId`）统一使用标准顶栏（`AppBar`）并隐藏底部导航
...
- 移动端影片详情与播放器尚未接入；播放列表详情页中的影片卡片点击当前仍使用 toast 反馈开发中状态
