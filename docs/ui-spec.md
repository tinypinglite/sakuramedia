...
- 当前菜单项自上而下依次为：
  ...
  2. `标记为合集/单体`
  3. `将"{prefix}"加入合集特征`（当番号能提取到合集特征前缀时才出现，并在其上方加分割线）
...
- 合集特征项会从当前番号中提取数字前完整前缀，例如 `OFJE-888 -> OFJE-`、`FC2-PPV-1234567 -> FC2-PPV-`；先读取 `collection-number-features`，若特征不存在则追加保存并立即触发合集重算
