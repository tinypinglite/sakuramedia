...
- 桌面端：已实现登录、工作台壳层、概览、女优上新、影片、女优、时刻、播放列表、排行榜、热评、搜索、详情、配置管理
- 移动端：已接入底部四导航（概览/影片/女优/榜单）与首页顶部 Tab（我的/关注/发现/时刻/热评）；“我的”已实现搜索栏、最近添加和播放列表，且已接入影片详情页、播放列表详情页、搜索子页；`/mobile/library/movies`、`/mobile/library/movies/:movieNumber`、`/mobile/library/actors`、`/mobile/library/actors/:actorId` 与 `/mobile/rankings` 均为真实页面
...
- “热评”Tab 复用热评列表页能力（周期切换、分页、空态/错误态）；点击热评卡片会入栈跳转到移动端影片详情，并携带 `/mobile/overview` 作为回退来源
