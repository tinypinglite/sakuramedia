...
- 移动端：已接入底部四导航（概览/影片/女优/榜单）与首页顶部 Tab（我的/关注/发现/时刻）；“我的”已实现搜索栏、最近添加和播放列表，且已接入影片详情页、播放列表详情页、搜索子页；`/mobile/library/movies`、`/mobile/library/movies/:movieNumber`、`/mobile/library/actors` 与 `/mobile/library/actors/:actorId` 已为真实页面
...
- 主导航页面（`/desktop/overview`、`/desktop/library/movies`、`/desktop/library/actors`、`/desktop/library/moments`、`/desktop/library/playlists`、`/desktop/system/configuration`）统一使用 `go`
...
统计信息使用 `OverviewStatsStrip`，当前包含系统汇总指标与 JoyTag 状态指标（健康、推理设备、待索引数量）；影片使用 `MovieSummaryGrid`。这是当前首页主模式，不要额外拼出第二套首页布局语言。
