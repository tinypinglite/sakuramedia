...
- 桌面端支持右键，移动端支持长按
- 当前覆盖：桌面影片列表、桌面发现页“女优上新”预览区块、桌面女优上新页、桌面女优详情影片列表、桌面标签影片列表、移动端影片列表、移动端女优详情影片列表
...
路由 `/desktop/library/clip-collections/:collectionId`。头部为合集名（旁带编辑铅笔）+ 切片数 + 「添加切片」「播放」按钮；下方为可拖序的切片行列表：

- 列表 `GET /clip-collections/{id}/clips`（按 `position` 升序，详情控制器一次性拉全所有分页）。
- 拖拽重排：本地重排后用 `PUT /clip-collections/{id}/clips` 提交完整有序 `clip_ids`（后端按列表重排 `position`），失败回滚并 toast。
- 行内「移出合集」：乐观移除 + `DELETE /clip-collections/{id}/clips/{clip_id}`，失败回滚。
...
媒体导入页是桌面端「管理」分区下的一级导航页，路由为 `/desktop/system/media-import`（Web 端复用桌面实现），用于把后端文件系统中已有的媒体导入到媒体库。后端接口集中在 `media-import` 标签下，导入作业跑在后台 `BackgroundTaskRun`（`task_key = media_directory_import`）。

页面结构：

- 顶部说明卡：左侧文案 + 右侧「刷新」「新建导入」按钮（`AppContentCard` + `AppButton`）
- 下方为导入作业卡片列表（按作业 id 倒序），滚动触底自动加载更多，底部复用 `AppPagedLoadMoreFooter`；无作业时用 `AppEmptyState`，初次加载用 `CircularProgressIndicator`
- 每个作业卡（`AppContentCard`）展示：源目录名/完整路径、状态徽标、导入方式、`导入/跳过/失败` 计数、创建/完成时间

实时进度规则：

- 页面独立订阅 task_run SSE（复用 `ActivityApi.streamEvents`），进入时先经 `/system/activity/bootstrap` 获取起始 `latest_event_id` 与活跃任务，避免 `after_event_id=0` 回放全量历史
- 仅消费 `task_key = media_directory_import` 的 `task_run_created` / `task_run_updated` 事件，按 `作业.task_run_id == task_run.id` 对齐
- 关联任务处于活跃态时，作业卡内联展示进度条（确定进度展示 `当前/总数`，否则展示不确定进度条 + 进度文案）；关联任务进入终态后自动刷新该作业计数与失败文件

新建导入对话框（`AppDesktopDialog`）：

- 顶部为目录浏览器：进入时浏览白名单根（多根概览可下钻、单根直接列内容），仅展示一层的子目录与视频文件；目录可下钻、视频文件灰显只读；`上一级` 在 `parent` 为空（已到白名单根）时禁用
- 当前浏览目录即导入源；底部为「导入到媒体库」（`AppSelectField<int>`，数据来自媒体库接口）与「导入方式」（`auto` 硬链接优先 / `cleanup-source` 复制后删除源文件）
- 仅当已进入某个具体目录且选定媒体库时才可「开始导入」；确认后触发导入并刷新列表，失败（含同库同源 `409` 冲突）以 toast 提示
...
非 JAV 视频域是与影片（catalog）平行的「仅播放 + 整理」体系，管理无番号、无外部元数据的视频。后端复用同一套播放底座（缩略图 / 播放进度 / 时刻 / 签名播放地址），前端因此**最大化复用影片侧组件**。在「浏览」分区下新增三个一级导航：`非 JAV 视频` `人物` `视频合集`；并把「管理」分区的 `媒体导入` 更名为 `JAV 媒体导入`，与之区分（前者走 `/import-jobs` 按番号抓元数据建影片，后者走 `/video-imports` 就地索引）。

- **视频列表页**（`/desktop/library/videos`）：顶部为「搜索标题 + 新建视频 + 导入」行；下方依次为标签筛选面板（复用 `TagSelectorPanel`，视频域多标签固定 OR、隐藏匹配模式开关）、人物筛选面板（`PersonSelectorPanel`，按关键词分页搜索）、排序工具条（`VideoFilterToolbar`：入库 / 发布 / 标题 × 升降序）与视频卡片网格（`VideoSummaryGrid` / `VideoSummaryCard`：封面 + 标题 + 媒体数角标 + 可播放角标）。分页复用 `PagedLoadController` + `AppPagedLoadMoreFooter`，标签 / 人物「已选项」变化即重载列表。
...
- **视频合集页**（`/desktop/library/video-collections`）：合集卡片列表（名称 + 描述 + 视频数 + 编辑 / 删除）+ 新建；合集详情（`/desktop/library/video-collections/:collectionId`）以 `ReorderableListView` 有序展示成员，支持拖拽**乐观重排**（失败回滚重载）、移除、单集 / 「播放全部」。
- **连播**：合集播放携带上下文（`collectionId` + 有序 `playlist` 视频 id），`MoviePlayerSurface` 新增可加性 `onCompleted` 回调，本集自然结束自动跳下一集，到末尾停止；非合集进入不跳转。
- **视频导入**：列表页「导入」按钮打开 `VideoImportDialog`，复用 `/filesystem/entries` 目录浏览选取源路径（目录或单视频文件），可选媒体库 / 标签 / 人物 / 合集，提交后 toast 汇报「新增 / 跳过」数。

新增共享组件：`lib/widgets/videos/`（`VideoSummaryCard` / `VideoSummaryGrid` / `VideoFilterToolbar`）；视频域专属选择器 `PersonSelectorPanel` 在 `lib/features/videos/presentation/`。`TagSelectorPanel` 新增可选 `showMatchModeToggle`（默认 `true`，影片侧不变；视频域传 `false`）。
...
- `ClipCollectionCard`：合集封面卡，`16:9` 封面（取首个切片封面）+ 名称 + 切片数角标，可选右上「···」编辑/删除菜单；用于切片页横滑区与合集列表网格。
