...
| **[data-loading.md](./data-loading.md)** | `AppPullToRefresh` / `AppAdaptiveRefreshScrollView` / `AppPagedLoadMoreFooter` / `AppFilterTotalHeader` / `AppText` / `MultiSelectStateMixin` / `SelectionCheckBadge` / `StaggeredLayout*` | 要接列表下拉刷新 / 分页脚 / 总数条 / 多选状态 / 交错布局 / 文字统一样式 |
| **[media-images.md](./media-images.md)** | `MaskedImage` / `AppImageFullscreenHost`(+`AppPinchToFullscreenImage`) / `MediaPreviewDialog` / `PreviewDialogSurface` / `PreviewImageStage` / `MediaPreviewActionGrid`(+`MediaPreviewActionTile`) / `AppImageActionTrigger` / `showAppImageActionMenu` / `resolveThumbnailGridColumns` | 要展示远端图 / 要建媒体预览浮层 / 要给图片挂右键长按菜单 |
