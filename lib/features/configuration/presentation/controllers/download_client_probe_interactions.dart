...
Future<void> handleProbeConnectivityTap({
  ...
  required DownloadClientProbe probe,
  ...
  final current = probe.currentState;
  if (current.busy) return;
  if (current.canReplayConnectivityDialog) {
    await openDialog(current.connectivityResult!);
    return;
  }
  try {
    final result = await probe.runConnectivity(runTest);
    if (!context.mounted || result == null) return;
    if (probe.currentState.connectivityChipState !=
...
Future<void> handleProbeStorageTap({
  ...
  final current = probe.currentState;
  if (current.busy) return;
  if (current.canReplayStorageDialog) {
    await openDialog(current.storageResult!);
    return;
  }
  try {
    final result = await probe.runStorage(runTest);
    if (!context.mounted || result == null) return;
    if (probe.currentState.storageChipState !=
        DownloadClientProbeChipState.healthy) {
      await openDialog(result);
    }
  } catch (error) {
