...
- 例外(合理越界):少数"对话框/检查器启动器"是胶水层,自己 `context.read<XxxApi>()` 组装控制器(如 `media/media_preview_dialog.dart`、`lib/features/movies/presentation/widgets/detail/movie_detail_inspector_panel.dart`——后者在 `initState` new 4 个子控制器并 dispose)。复用时注意它们会**真实发起请求**,不是哑组件。
...
2. **`lib/features/movies/presentation/widgets/detail/movie_plot_thumbnail.dart` `MoviePlotThumbnail`** — 剧情图专用(`ResizeImage` 按高解码 + 监听真实宽高比)。**与 `MaskedImage` 不可互换**:列表封面用它会多开销,剧情图用 `MaskedImage` 会丢比例。
...
- **弹窗**:顶层 `showXxxDialog(...)` 函数包 `showDialog`/`showAppBottomDrawer`。media_kit 播放弹窗(`ClipPlayerDialog`/`VideoQuickPlayDialog`)仍用 `showDialog` + `AppDesktopDialog` 包壳,但**播放器本体统一走 `media_player/themed_video_player.dart` 的 `ThemedVideoPlayer`**(见下)。
- **播放器控制组件两层复用(统一入口)**:所有视频播放只走这两层之一,别再写裸 `Video` / 自己嵌三层控制主题。
  - **层级一(重)`MoviePlayerSurface`**:仅影片应用内播放页用,自持 Player + 字幕 / 进度上报 / 缩略图圈选 / 播放信息。
  - **层级二(轻)`ThemedVideoPlayer`**:其余全部播放入口(快播弹窗、单切片 / 单视频全屏页、切片 / 视频合集连播页)。调用方自建 `Player`/`VideoController` 传入,顶 / 底控制条按场景拼(合集有上一首 / 下一首,单片 / 弹窗没有);组件内部复用 `media_player/movie_player_surface.dart` 的 `buildMoviePlayerXxxControlsThemeData` + `resolveMoviePlayerVideoControlsBuilder` 套统一主题(进度条 / 全屏 / 音量样式一致)。
...
- **合集连播页布局 = 左右分栏(左播放器 + 右「整部合集」关键帧面板)**:两个合集连播页(`desktop_{clip,video}_collection_play_page.dart`)`build` 用共享壳 `media_player/collection_play_split_layout.dart` `CollectionPlaySplitLayout`(`Area(0.72)/Area(0.28)` + `DividerPainters.grooved1`,语义对齐 jav `_MoviePlayerSplitLayout` 但**独立实现**,jav 播放页不迁移)。左 area = 原沉浸式 `Stack[player + EpisodeSelectorOverlay]`(选集浮层原样保留在左面板内),右 area = 缩略图面板。
  - 两个连播页 State 都 `with CollectionPlaybackPageMixin`(`media_player/collection_playback_page_mixin.dart`):**共享**播放器登记(`attachPlayback`)、`dispose`、跨集 seek 补偿(`seekToFrame`/待办 `_pendingSeek` 一次性 position 监听)与右面板构建(`buildFilmstripPanel`)。各页只留 `_load`/剧集列表/底栏/`_QueueItem`。**注意**:`updatePosition`/同集判定都用播放器**实时** `player.state.playlist.index`(非滞后的 `currentIndex`,position 流可能先于 playlist 流到);`seekToFrame` 覆盖待办时会先取消旧的一次性监听(防旧 offset 误 seek)。
  - **右面板「整部合集」关键帧**:把整个合集拉平成「一部完整长片」——按集顺序无缝拼接第 1…N 集全部关键帧成一条连续序列,由 `media_player/collection_filmstrip_controller.dart` `CollectionFilmstripController` 聚合(渐进逐集加载、**优先拉起播集** `start(priorityEpisode:)`、`updatePosition(集,集内秒)`→全局活动帧高亮(位置早于该集首帧则不高亮)、`resolveTarget(全局帧)`→所属集+集内秒;`thumbnails` 由 `_frames` 惰性派生并缓存)。**复用 jav 的 `MoviePlayerThumbnailPanel`/`MovieMediaThumbnailGrid`**(不传圈选回调→圈选 UI 隐藏)。每帧携带真实 `mediaId/thumbnailId`(pornbox 来自媒体缩略图填真值,切片无对应 media 填 `0`),`thumbnails` DTO 据此透传。点帧跨集跳转走「`jump` 后待目标集首个 position tick 再 `seek`」补偿(media_kit `jump` 后立刻 `seek` 会失效)。数据源:切片走 `ClipsApi.getClipThumbnails`(切片自身时间轴 offset)、pornbox 走 `MoviesApi.getMediaThumbnails(firstMediaId)`(媒体自身时间轴,整段从 0 播)。**注意它和底栏「选集」浮层是两回事**:选集 = 每集一张封面的播放列表;关键帧面板 = 把合集当一部长片的逐帧进度条。
    - **右键/长按某帧「添加时刻」**(仅 pornbox):时刻 = MediaPoint(`MediaApi.createMediaPoint(mediaId, thumbnailId)`),故只有携带真实 media/thumbnail id 的 pornbox 帧支持——`buildFilmstripPanel(onThumbnailMenuRequested:)` 由 `desktop_video_collection_play_page` 传入菜单回调(切片页不传→无菜单)。菜单先 `getMediaPoints` 查该帧是否已是时刻,据此显示「添加/删除时刻」,桌面用定位弹窗、移动(`useTouchOptimizedControls`)用底部抽屉,操作走 oktoast。移动连播页是桌面页薄壳,自动两端覆盖。**对齐 jav 播放器缩略图的「添加标记」,但不广播**(无 MomentMutationNotifier,时刻库下次导航自然刷新)。
- **合集连播「选集」浮层 `media_player/episode_selector_overlay.dart`**:切片 / 视频合集连播页底栏(全屏按钮左侧)的「选集」按钮(`MaterialCustomButton`/`MaterialDesktopCustomButton`)唤出 `EpisodeSelectorOverlay`——右侧滑出剧集面板、点遮罩或选集关闭、展开时自动滚动定位当前集。两个合集页共用此组件,各传 `itemBuilder`(封面 + 名称 + 高亮 + 点击跳转)。**与影片播放器右侧的「视频信息」抽屉(`buildMoviePlayerInfoSideDrawerOverlay`)是两回事,互不复用、勿混改。**「选集」浮层现位于**左分栏面板内**(Video 的兄弟节点),media_kit 全屏会 push 独立路由把它盖住——故底栏「选集」按钮**只在窗口态出现**(`ThemedVideoPlayer.fullscreenBottomControls` 传一份去掉该按钮的列表),全屏态不放,避免死按钮;换集需先退出全屏。`EpisodeSelectorOverlay` 行高(`itemExtent`)随系统字体缩放放大,防双行标题在放大字号下溢出。
