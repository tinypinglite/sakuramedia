...
<h2 align="center">当前进度与更新计划</h2>

**当前已实现功能**

- [x] 跨平台APP支持，支持Mac、Windows、Ios(需自行签名安装)、Android
- [x] 登录与会话基础流程。
- [x] 桌面端工作台壳层、概览页与全局搜索入口。
- [x] 影片列表、影片详情、影片播放与系列影片页。
- [x] 女优列表与女优详情页。
- [x] 女优上新与订阅入口。
- [x] 时刻、播放列表、排行榜、热评与活动中心入口。
- [x] 配置管理入口。
- [x] 移动端基础路由与部分页面骨架。
...
- [ ] 支持按年份和标签筛选影片。
- [ ] 支持查看导入失败的影片，并支持手动导入。
- [ ] 基于 Cloudflare Workers 共建影片元数据信息，包括影片番号、女优、点赞、评论等信息。
- [ ] 迁移数据源至 DMM，互动数与影片热度信息基于共建的影片元数据信息库，不再依赖 JavDB。
